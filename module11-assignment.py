...
def plot_category_performance_by_location():
    latest_q = sales_df['QuarterLabel'].iloc[-1]
    data = sales_df[sales_df['QuarterLabel'] == latest_q]

    pivot = data.pivot_table(values='Sales', index='Category', columns='Location')
...
def plot_sales_by_price_tier():
    ...
    explode = [0.1 if val == grouped.max() else 0 for val in grouped]

    fig, ax = plt.subplots()
    ax.pie(grouped, labels=grouped.index, autopct='%1.1f%%', explode=explode)
...
def create_business_dashboard():
    ...
    grouped = sales_df.groupby(['QuarterLabel', 'Location'])['Sales'].sum().unstack()
    for col in grouped.columns:
        axes[0, 1].plot(grouped.index, grouped[col], label=col)
    axes[0, 1].legend()
    ...
    axes[1, 1].pie(cat, labels=cat.index, autopct='%1.1f%%')
...
print("""
1. Sales show an upward trend with spikes in Q4.

2. Miami is more efficient than other locations, suggesting higher market potential.

3. Electronics are the most profitable across all regions, making it the most profitable category.

4. There is a positive relationship between advertising spend and sales, ensuring marketing effectiveness.

5. Mid-range pricing generates the highest revenue, indicating it is the best tier for customers.

6. Younger customers (18–30) tend to spend less per purchase compared to older customers.

RECOMMENDATIONS:
- Increase advertising budget strategically.
- Expand most efficient categories like Electronics.
- Focus growth efforts in Miami and replicate its strategies in other cities.
- Target mid-range pricing strategies to maximize profit.
""")
